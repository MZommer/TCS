...
// Connection
const EnviromentModel = sequelize.define("Enviroment", {
    ID: {
...
const SpaceModel = sequelize.define("Space", {
    SpaceID: {
        type: DataTypes.STRING,
        primaryKey: true,
        allowNull: false,
...
module.exports = class DB {
    static async getEnvConfig(EnvID) {
        const env = await EnviromentModel.findOne({
            where: {ID: EnvID}
        })
        return env
    }
    ...
    static async createSpace(SpaceID, EnvID){
        return SpaceModel.create({SpaceID, EnvID}).catch(console.error)
    }
